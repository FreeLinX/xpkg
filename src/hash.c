...
/* Writes a 65-byte (64 hex chars + NUL) lowercase hex digest of a NUL-
 * terminated string into out. Used to fingerprint symlink entries (the
 * digest of the link target), which `xpkg verify` re-checks via readlink. */
